...
from sqlalchemy import cast, DateTime, func, extract, String
...
@router.get("/users/{user_id}/{type}")
def user_data(user_id: UUID, type: str, db: Session = Depends(get_db)):

    if type == "assigned":
        return db.query(Card).filter(Card.assigned_to == str(user_id)).all()
...
def analytics_top_users(db: Session = Depends(get_db), admin=Depends(get_admin)):
    ...
    q = (
        ...
        # .join(Card, Card.assigned_to == cast(User.id, String))
...
def admin_tasks(
    ...
    if hasattr(c.due_date, "isoformat")
    else c.due_date
) if c.due_date else None,
            # ✅ FIXED STATUS
            "status": "Done" if c.completed_at
            else ("In Progress" if c.assigned_to else "To Do"),
        })
